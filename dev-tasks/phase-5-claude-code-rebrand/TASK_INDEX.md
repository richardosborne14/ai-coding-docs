...
- **Added 2026-09-21 (`ae60c04`, live):** `part-6/nodegx.md`, the NodeGX case study (own sidebar entry under Resources, teaser on case-studies). Numbers come from the OpenNoodl repo on 2026-09-21 (git log/tags, dev-docs, memory count). If refreshed later, re-measure; don't edit numbers by hand. Richard approved it on a local preview. The Low Code Foundation link is now helloasso (site-wide).
- **Added 2026-09-23 (`eebf040`, pushed, not deployed):** a "Copy page as Markdown / Download .md" bar on every doc page. The build writes `<page>.md` next to each `.html` and `/llms.txt` (`docs/.vitepress/llm-markdown.ts`). `learn-ai.caddy` now serves `*.md` as `text/markdown`, so the next deploy reloads Caddy with that change. Clipboard copy not yet clicked in Safari.
